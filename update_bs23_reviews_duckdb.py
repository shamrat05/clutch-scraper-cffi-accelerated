...
def rebuild_all_reviews():
    print("[+] Extracting ALL reviews from Brain Station 23 HTML...")
    ...
    print(f"[✔] Found {len(bs23_reviews)} real client decision-maker reviews for Brain Station 23!")
    ...
    print("[+] Rebuilding DuckDB reviews table...")
    ...
    print(f"[+] Inserting {len(review_records):,} total review records into DuckDB...")
    ...
    # Check Brain Station 23 in reviews table
    bs23_in_reviews = conn.execute("SELECT reviewer_name, reviewer_company, review_title FROM reviews WHERE LOWER(vendor_agency_name) LIKE '%brain%station%'").fetchall()
    print(f"\n[✔] Verified {len(bs23_in_reviews)} reviews now in reviews table for Brain Station 23:")
    for r in bs23_in_reviews:
        print(r)
